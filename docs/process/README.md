...
| [cut-gate-cursor.md](cut-gate-cursor.md) | **CURRENT (Cursor)** — the **full standalone** Cursor counterpart to cut-gate.md: every section present (0-cut-the-release through §9-portability, incl. the digest/persona/temporal-validity headline sections), runnable end-to-end including the tag. Swaps the Claude-Code surfaces for the three `.cursor/` ones (`.cursor/hooks.json` version-1 + the single `cmk cursor-hook` dispatcher on all six events, `.cursor/mcp.json`, the `.mdc`-required always-applied rule), verifies the six hooks FIRE in a real Cursor session (inject / capture-prompt / capture-turn / observe-edit / compress / delete-guard), the Cursor-aware doctor HC-1, and the Cursor uninstall/dual-agent lifecycle. Bakes in the **D-262 restart-before-verify** rule (Cursor is a VS Code fork — hooks load at session start) + the D-269 inject-content guard. The agent-neutral sections (capture, digest, persona, temporal-validity, feature-sweep, portability) run as-is against the files a Cursor session produces. Flags one honest kit gap: **HC-9 version-drift is N/A on a Cursor-only install** (it reads the CLAUDE.md marker, which Cursor doesn't write). The Task-196 Cursor live-test. |
