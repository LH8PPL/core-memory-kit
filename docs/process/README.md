...
| [cut-gate-kiro.md](cut-gate-kiro.md) | **CURRENT (Kiro)** — the Kiro counterpart to cut-gate.md: a full mirror with every Claude-Code surface swapped for its Kiro equivalent (`.kiro/`, `.kiro.hook` IDE hooks, the Amazon-Q CLI agent-config + guarded default, kiro-cli + Kiro IDE clients). IS the v0.4.0 Kiro live-test (sub-task 50.M). Sandbox-binding: always run with `MEMORY_KIT_AWS_DIR` + `MEMORY_KIT_USER_DIR` set (D-184). |
