...
Concrete example from `claude-memory-kit`: the user works in a regulated banking environment. Claude.ai's memory knows this. When the user asked Claude.ai to research memory systems for a **personal open-source project**, the research output assumed bank/air-gap context throughout — recommending Bedrock adapters, audit logs, machine-pinned tiers, none of which the personal project needs.

The user identified this as a recurring pattern: *"claude ai research does that all the time to me because he know i work at an airgapped/bank environment, and sometimes he makes assumptions that i didn't ask him to do."*
...
2. Find the specific entry ("user works in banking" or similar).
