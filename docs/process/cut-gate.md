...
```powershell
mkdir C:\Temp\cut-gate9; cd C:\Temp\cut-gate9
git init
...
git -C C:\Temp\cut-gate9 check-ignore context\sessions\probe.extract.log
# The trace half (observational): if any build turn was graded LOW, you'll see it.
findstr /S /C:"low_trust_discarded" C:\Temp\cut-gate9\context\sessions\*.extract.log
```
...
```powershell
mkdir C:\Temp\cut-gate-coldopen9; cd C:\Temp\cut-gate-coldopen9
...
## 7. Full feature sweep — every `cmk` subcommand  (~20 min, in `C:\Temp\cut-gate9`)
...
- [ ] **F-19 — `uninstall` is clean + `init-user-tier` re-seeds (lifecycle, in the throwaway dir ONLY).**
      In `C:\Temp\cut-gate9` (NEVER a real project):
      ```powershell
...
In `C:\Temp\cut-gate9`: `git add -A; git commit -m "wip"`.
Clone elsewhere (`git clone C:\Temp\cut-gate9 C:\Temp\cut-gate-clone`), open *that* in Claude Code.
