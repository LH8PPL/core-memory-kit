...
- **Claude Code CLI reference**: <https://code.claude.com/docs/en/cli-reference> — authoritative flag list for the `claude` binary the Haiku compressor spawns. Checked 2026-06-03 (Lior pointed here): confirmed `--print/-p`, `--model`, `--max-turns` (print-mode only), `--mcp-config`, `--strict-mcp-config`, `--output-format text` are all correct in [compressor.mjs](../packages/cli/src/compressor.mjs). **Surfaced a real finding (D-43 / Task 88):** `--allowedTools` is an *auto-approve allowlist* (*"execute without prompting … to restrict which tools are available, use `--tools` instead"*), NOT an availability restriction — so the kit's `--allowed-tools ''` does not enforce the "tightest sandbox" its comment claims; `--tools ""` ("disable all") is the correct flag. Live exposure ≈ nil (print + max-turns 1 has no approver), but the flag choice + comment are wrong — fix tracked as Task 88.
