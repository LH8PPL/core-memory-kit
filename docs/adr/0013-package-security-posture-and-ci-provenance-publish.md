...
- **SonarCloud (hosted SonarQube)** for SAST — offered (free for public repos; matches Lior's work tooling). Lior chose **CodeQL** (GitHub-native, no external account/infra). SonarCloud's *quality/coverage/maintainability* dimension is a distinct concern, split to candidate Task 54, not folded into security.
...
- **Keep local publish, add scans only** — rejected by Lior in favor of doing the full hardening (incl. provenance) before v0.1.1.
