...
| [0022](0022-redact-scrubs-app-layer-never-git-history.md) | The Task-96 compliance scrub: `cmk redact` scrubs the APP layer completely (live fact + every tombstone/superseded/archive copy + indexes, audit-preserving) and prints an honest git advisory (rotate first; the exact path-scoped `filter-repo` as a documented one-time team operation) — the kit NEVER rewrites git history (force-push breaks the D-27 portable-clone niche; no surveyed system auto-rewrites); `cmk purge --hard` stays CLI-only/never-MCP (§6.5) | proposed | 2026-07-16 |
