...
| [0012](0012-npm-publish-name-and-cross-agent-future.md) | Publish v0.1.0 under `@lh8ppl` scope; defer the cross-agent product name to v0.2 | **superseded by [0021](0021-rename-to-core-memory-kit.md)** | 2026-05-29 |
...
| [0018](0018-memory-commit-propose-and-approve.md) | Memory commit — the kit DETECTS accrued uncommitted `context/` memory and PROPOSES (a model-facing SessionStart line); the user one-tap approves; git stays agent-run under the host permission model, never kit-run (reconciles the no-auto-git position; basic-memory-validated; Task 150) | accepted | 2026-07-02 |
| [0019](0019-auto-judged-privacy-screen.md) | Auto-judged privacy — a two-layer screen (L1 deterministic PII patterns sync + mask-in-place-pre-hash; L3 async Haiku judge in the detached child, Anthropic PII-purifier-based) at two boundaries (transcript tier → gitignored live-buffer→judge→promote, fail-closed; fact path → sensitivity commit\|local-only\|drop axis); tier-routing-as-quarantine + the gitignored redactions.log recovery are field-novel (Task 148, the v0.5.0 cold-open incident) | accepted | 2026-07-07 |
| [0020](0020-incremental-resumable-long-jobs.md) | Long jobs are incremental + resumable-from-artifacts, never all-or-nothing — a killed-at-80% run persists the 80% and resumes; the resume point is DERIVED from the persisted artifacts (which units already appear in the output), never a new sentinel file (ADR-0002); a third strategy alongside bound-input (D-173) + derive-from-artifacts; `daily-distill` is the reference beneficiary (per-day append, resume at the first date not in recent.md); the review checklist graduates to CLAUDE.md (Task 203/204, the D-298 starvation bug) | accepted | 2026-07-11 |
| [0021](0021-rename-to-core-memory-kit.md) | Rename the project to `core-memory-kit` — executes ADR-0012's deferred cross-agent name (trigger fired: Kiro/Cursor/Codex all live); keeps `cmk` (the name was chosen to preserve the initialism); npm deprecate-with-pointer + GitHub repo rename + a carve-out corpus sweep; the user-tier config dir migrates copy-not-move / marker-gated / keep-the-old (never a text swap — the highest-risk line). Supersedes ADR-0012 (Task 195) | accepted | 2026-07-14 |
| [0022](0022-redact-scrubs-app-layer-never-git-history.md) | The Task-96 compliance scrub: `cmk redact` scrubs the APP layer completely (live fact + every tombstone/superseded/archive copy + indexes, audit-preserving) and prints an honest git advisory (rotate first; the exact path-scoped `filter-repo` as a documented one-time team operation) — the kit NEVER rewrites git history (force-push breaks the D-27 portable-clone niche; no surveyed system auto-rewrites); `cmk purge --hard` stays CLI-only/never-MCP (§6.5) | accepted | 2026-07-16 |
| [0024](0024-recall-architecture-judgment-pulled-hybrid.md) | Recall architecture (Task 149, the 15-system trigger study): KEEP judgment-pulled as the deep spine — the recall model follows loop ownership (loop-owners pipeline; MCP servers are structurally pulled; every Claude-Code-plugin peer converges on the kit's hybrid, and the one real per-prompt semantic inject ships default-off); Memora validates the LADDER not the trigger. Close the under-fire risk with three composed borrows: a GATED cheap-index pointer hint (FTS5 top-3 index lines behind a score floor — pro-workflow + graphiti patterns), Letta's existence-advertisement metadata in the snapshot, and the in-skill Memora refinements (score-then-fetch, episodic grouping via expand, the RE_QUERY rule). Per-prompt SEMANTIC injection and inject-everything rejected with reasons | accepted | 2026-07-19 |
| [0023](0023-graph-recall-activate-edges-defer-derivation.md) | Graph recall (Task 176, the 9-system code sweep): ADOPT the S-sized ACTIVATION slice — index the `related:`/`superseded_by` edges the kit already writes (rebuildable `edges` table + recursive supersession CTE + `cmk links`/`mk_links` + successor-naming labels; the edges are invisible today, even `mk_get` omits them); DEFER LLM derivation (Memora `cues:` first in line) behind a measurable trigger — add a relational qtype to the Task-99 bench, numbers decide; REJECT graph-DB servers / typed-KG stores / hand-authored graphs (drift, second-source-of-truth, the pulse8 edges-outside-markdown data-loss shape) | accepted | 2026-07-19 |
