...
- We do not adopt the newer experimental hook events (InstructionsLoaded, UserPromptExpansion, Elicitation, PostToolBatch, WorktreeCreate, TaskCreated, TeammateIdle). Those churn frequently per Option-B research; v0.1 sticks with the stable events.
- **Update (Task 74, 2026-06-30, D-218) — re-injection after compaction is ALREADY HANDLED, so no PreCompact hook is needed.** The original note lumped `PreCompact` in with the experimental churn; that was over-cautious. PreCompact/PostCompact are documented stable now — but more importantly, the kit does not need them: Claude Code fires **`SessionStart` with `source:"compact"`** after an auto-compaction, and the kit registers `SessionStart: cmk-inject-context` with **NO `source`/`matcher`** (settings-hooks.mjs), so the matcher-less hook re-runs `cmk-inject-context` and re-injects the frozen snapshot post-compact. `inject-context.mjs` is source-agnostic (it never reads the `source` field), so the re-fire restores memory regardless of WHY SessionStart fired. This is the kit's own thesis applied to compaction — the very failure it exists to kill — and it works through the existing SessionStart wiring, not a new event. **Locked in** by `cli-install-hooks.test.js` ("SessionStart re-injects post-compaction: matcher-less hook") + `cli-inject-context.test.js` ("injectContext source-agnostic"). _Honest caveat: verified against the Claude Code hooks doc + the kit's code + the lock-in tests; a LIVE compaction confirmation (trigger a real auto-compact, see the snapshot reappear) is the manual cut-gate step._
