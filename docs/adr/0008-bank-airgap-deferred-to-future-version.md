...
- The user, who actually works in a regulated environment, has a future path if they later want to deploy this at work — but it's not blocking v0.1.
...
|---|---|
| Build bank/air-gap features in v0.1 | Scope explosion. The user isn't deploying this at work in 2026; building the feature now is speculative. |
...
| Drop bank/air-gap entirely from the roadmap | The user actually does work in that environment. A future contribution path matters. v0.2+ is the right time. |
