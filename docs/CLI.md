...
- `--ide cursor` → removes the **Cursor** surface: the kit's events in `.cursor/hooks.json` (your own hooks preserved), the `claude-memory-kit` entry in `.cursor/mcp.json` (sibling servers preserved), and the managed block in `.cursor/rules/claude-memory-kit.mdc`.
- `--ide codex` → removes the **Codex** surface: the kit's events in `.codex/hooks.json` (your own hooks preserved), the `claude-memory-kit` MCP entry via `codex mcp remove` (only when the project shows a kit install — a clean project is a quiet no-op), and the managed `AGENTS.md` block.
