...
- **Resumable + idempotent**: each session persists as it completes; the committed `context/sessions/imported-sessions.md` ledger is the resume point, so a killed run keeps its progress and a re-run imports only NEW sessions.
