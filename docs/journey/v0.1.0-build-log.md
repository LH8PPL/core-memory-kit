...
**Environment footnote.** WSL `bash.exe` corrupted on the dev machine mid-task (`execvpe /bin/bash failed 2`), spuriously failing ~35 hook-bin tests that `spawnSync('bash')`. Diagnosed as environment (Git Bash + CI Linux both clean), not code; ran the gate under Git Bash on PATH (1276/1276, stress 5/5). Logged in DECISION-LOG so the next session doesn't chase it in code.
