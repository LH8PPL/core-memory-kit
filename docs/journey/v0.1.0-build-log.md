...
| **PR-C** | `audit-spec-stack-references` | **OPEN** (PR #34) | Cross-reference rot audit (§X.Y / ADR-NNNN / FR-NN / NFR-N / Task NN / `[label](path)`) + numbering-gap audit (ADR 0009/0010, others) + evidence-driven ADR backfill from `docs/research/` + `docs/sources/` + `docs/adr/` + `docs/conversation-log/` + git history + `SOURCES.md` + CLAUDE.md "primary-source verification" amendment (internal references are subject to the same rule as external citations) |
| **PR-D** | `audit-completeness-and-enforcement` | Queued behind PR-C | Composition audit + class-5 exit-doors audit on ~22 remaining test files + `@doors:` annotation pass on every kit test + enforcement validators (5 new) + CLAUDE.md "Prose rules vs enforcement" section + design.md §17.7 "Enforcement validators for §17 disciplines" + campaign wrap-up journey-log entry |
| **PR-E** | `audit-cross-platform-portability` | Queued behind PR-D | Class-7 cross-platform discipline sweep. **Full scope inlined below** (PR-E §full-scope subsection) because the verbatim phrasings (10th meta-rule text, live-test scenarios, scripts/shebang enumeration) are load-bearing — captured here so PR-E doesn't re-derive less-precise versions from compacted context. |
