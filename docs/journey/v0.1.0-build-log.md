...
| **PR-E** | `audit-cross-platform-portability` | Queued behind PR-D | Class-7 cross-platform discipline sweep. Surfaced by PR-B's `recoveryCommand` hardcoded `rm` (Windows users get a broken hint on stock cmd.exe). **Release blocker for v0.1.0** — Lior runs Windows + macOS + Linux at different machines/contexts; every user-facing shell command needs to be platform-aware (programmatic emissions) or platform-explicit (documentation). Scope: every emission site (`lock-discipline.mjs recoveryCommand`, `cmk doctor` `HC-*` repair commands, `cmk repair` output, error messages saying "run X to fix", install.mjs completion paths) + every doc with shell commands (README install, SETUP.md, HEALTH-CHECKS.md repair, design.md user-facing, tasks.md commands, ADR examples, template CLAUDE.md) + spec stack updates (design.md §18 or §17.8 "Cross-platform discipline"; CLAUDE.md 10th meta-rule; tasks.md retroactive sub-tasks; new `validate-platform-commands.mjs` joining the enforcement set; possibly HC-10 "all `HC-*` recovery commands are platform-aware"). Pattern: `process.platform` switch for programmatic; POSIX + PowerShell side-by-side for docs; `node -e "fs.rmSync(...)"` for cross-platform single-commands. Don't sweep internal `process.platform` checks (already correct per PR-A). |
