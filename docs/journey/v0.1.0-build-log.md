...
| **PR-D** | `audit-completeness-and-enforcement` | **NEXT** (queued behind PR-C now that PR-C is merged) | Composition audit + class-5 exit-doors audit on ~22 remaining test files + `@doors:` annotation pass on every kit test + enforcement validators (5 new) + CLAUDE.md "Prose rules vs enforcement" section + design.md §17.7 "Enforcement validators for §17 disciplines" + capture-turn spawn-failed observability fix (structurally deferred from PR-B) + skill experiment with audit-note template + Adoption-verification sub-rule + campaign wrap-up journey-log entry |
