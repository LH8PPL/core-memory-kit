...
### Task 33 — RESUME HERE (in-flight, branch `task-33-daily-distill`)

**Status as of compact 2026-05-28**: spec stack updated + initial impl partially landed. Branch is at commit `b12152f` (pushed to remote).

**Decisions locked this session**:

- **Option A** confirmed by Lior — ship full cross-platform cron registration in v0.1.0 (Linux + macOS + Windows). Lior: *"I need it to work on Windows and Mac out of the box, so adding Linux is a small thing."*
- **Node.js, not Python** — `scripts/register-crons.mjs` (NOT `register-crons.py`). 4-point rationale in design §8.6.3. Both options preserved in tasks.md 33.2 per the new decision-trail rule.
- **Two-pass code-review** still required before merge (self-review then code-review-excellence skill) per CLAUDE.md Workflow.

**Already in branch `task-33-daily-distill`** (commits `3caa2b8` + `b12152f`):

1. `specs/v0.1.0/design.md` §8.6 (NEW) — full architecture: §8.6.1 module shape, §8.6.2 cross-platform mapping, §8.6.3 Node-vs-Python decision, §8.6.4 cooldown + audit-log composition
2. `specs/v0.1.0/tasks.md` Task 33.2 — decision trail (both Python + Node options visible, rationale, cross-platform mapping table)
3. `packages/cli/src/daily-distill.mjs` (NEW) — public boundary `dailyDistill({projectRoot, backend, now, cooldownMs?, maxOutputBytes?})`:
   - Reads last 7 days of `context/sessions/today-{YYYY-MM-DD}.md` (date math, not mtime)
   - Composes on cooldown.mjs (`isCooldownActive` gate + `touchCooldownMarker` on success+error per §8.2)
   - Passes `timeoutMs: 50_000` to CompressorBackend per §8.5
   - Writes `context/sessions/recent.md` (full overwrite — atomic-rename v0.1.x candidate)
   - NDJSON `{date}.distill.log` with same schema family as compress/extract
4. `CLAUDE.md` decision-trail preservation rule (commit `b12152f`)

**STILL TO DO on this branch before PR-merge**:

1. `packages/cli/src/register-crons.mjs` — cross-platform scheduler registration:
   - Linux: `crontab -l | (grep -v cmk-daily ; echo "0 23 * * * ...") | crontab -` (pipe pattern, idempotent)
   - macOS: write `~/Library/LaunchAgents/com.cmk.daily-distill.plist` + `launchctl bootstrap gui/$UID <plist>`
   - Windows: `schtasks /Create /TN cmk-daily-distill /SC DAILY /ST 23:00 /TR "node <path>" /F` (`/F` for idempotent re-create)
   - `--dry-run` flag (prints command without executing — used by tests + by users who want to inspect before granting host permissions)
   - `--unregister` flag (`crontab -l | grep -v cmk-daily | crontab -`, `launchctl bootout` + `rm`, `schtasks /Delete /F`)
2. `plugin/bin/cmk-daily-distill.mjs` — bin wrapper invoked by cron. Instantiates HaikuViaAnthropicApi, calls dailyDistill().
3. `packages/cli/src/subcommands.mjs` — wire `cmk daily-distill` + `cmk register-crons` verbs (currently both stubs). Also handle `--dry-run` / `--unregister` options.
4. `tests/cli-scaffold.test.js` — add `daily-distill` and `register-crons` to NON_STUB_VERBS allowlist (same pattern as Task 26's `queue/review` + Task 30's `search`).
5. `tests/cli-daily-distill.test.js` — 4 cases per tasks.md 33.4:
   ...
   - Test cooldown active: script exits `skipped: cooldown`; no recent.md written
   - Test 0 `today-*.md` files: script exits 0 cleanly; recent.md unchanged
6. `tests/cli-register-crons.test.js` — cross-platform dry-run tests (assert command output for current platform; assert idempotent re-run output)
7. Self-review + skill-review (subagent) per CLAUDE.md Workflow's two-pass discipline
8. Fix all findings inline per Lior's standing "fix everything now" directive (unless v0.1.x — §16 entry)
9. Update PR body with both review summaries
10. `gh pr merge --squash --delete-branch` (autopilot auto-merge authority granted by Lior for Tasks 28-33)
11. Pull main + flip Task 33 checkboxes + journey log retrospective + push direct to main
12. **Stop autopilot** — Task 33 is the LAST in Lior's range. After merge, surface to Lior for next-direction call (Tasks 34-35 Layer 6 continuation, OR skip to cross-cutting Tasks 37-43, OR v0.1.0 release prep).

**Key context for post-compact me**:

- Branch `task-33-daily-distill` is at `b12152f` (pushed). Not yet a PR.
- 963 tests / 46 files / 8 validators currently green on main. The branch's tests haven't been added yet but the daily-distill.mjs module exists and should be syntactically valid (typecheck via `node -e "import('./packages/cli/src/daily-distill.mjs').then(m => console.log('loaded:', Object.keys(m)))"` if uncertain).
- Stress gate (fast-mode 5/5 first invocation, OR documented live-Haiku jitter exception per CLAUDE.md Workflow) is the merge gate per kit discipline.
- The kit's existing `cooldown.mjs`, `compressor.mjs` (HaikuViaAnthropicApi), `audit-log.mjs::nowIso`, `result-shapes.mjs` (ERROR_CATEGORIES) are the shared modules the implementation composes on. Don't re-implement these — import them.
- `schtasks` on Windows handles the `/F` flag for idempotent re-creates. `crontab` pipe pattern + `grep -v cmk-daily` is the Linux idempotency primitive. `launchctl bootstrap` is the macOS load-or-replace primitive.
- Per the autopilot stop rule, anything that ACTUALLY registers a cron on the developer's machine is OUT OF SCOPE. Tests should `--dry-run` only.

**Session methodology directives from Lior (2026-05-27, must persist)**:
