...
- **Task 26 (in-flight, branch `task-26-review-queue`, durability-committed as `436561c`)** — Review queue + `cmk queue review` resolver (T-023). `review-queue.mjs` module with `parseReviewQueue` + `resolveReviewQueue` (promote / discard / skip). Wired through `cmk queue review` via `runQueueReview` in `subcommands.mjs`. 12 base tests PASS + 1 IMP-1 test currently FAILING (asserts behavior that doesn't happen — see follow-up below). Branch pushed to remote; PR not opened yet.

**Task 26 follow-up needed (next session)**:

1. **Failing test in `tests/cli-review-queue.test.js`** — the test "promote that conflicts with existing high-trust → re-routed to conflict-queue" asserts the promoted bullet ends up in conflicts.md, but the actual behavior is "both bullets in MEMORY.md" because `memory-write.doAdd`'s conflict-queue route only fires when `new.trust < existing.trust` (queue route), and promote always sets `trust: 'high'` so the supersede path fires instead — which currently falls through to normal append (auto-mutate of existing bullet's `superseded_by:` is deferred to v0.1.x per `memory-write.mjs` comments). Fix: either update the test to assert actual supersede behavior, OR delete it as a false-positive code-review finding. The defensive `rerouted_to` handling in `review-queue.mjs` stays as future-compat for v0.1.x routing changes.
2. **Minor #5 not yet addressed** — CLI stdin-emulation test for `runQueueReview` (parallel to the parked test for `runQueueConflicts`). Lior's directive: "fix every code-review finding now". Owed in next session.
3. **Stress gate not run** — context pressure interrupted before `npm run stress 5x`.
4. **PR not opened** — task-26-review-queue branch is on remote but `gh pr create` hasn't fired.
...
- **CLAUDE.md "Integration-test coverage for cross-module flows" (binding meta-rule)** — added next to the "Composition verification" rule. Source: Task 25's latent `generateId({text, tier})` named-args bug that the kit's full test suite missed because every queue-route test bypassed `memory-write` and constructed `writeConflictEntry` directly. Task 25b's `mergeScratchpadBullets` work added the missing memory-write → conflict-queue integration test that would have caught the bug originally. The rule generalizes: per-module unit tests answer "does the surface work?"; integration tests answer "do the surfaces COMPOSE?" — both required for cross-module flows.
