...
1. ✅ **SHIPPED — v0.1.2 is LIVE on npm (2026-05-30)** as `latest`, SLSA-provenance-signed (`predicateType: slsa.dev/provenance/v1`). PR #79 merged → tag `v0.1.2` pushed → `publish.yml` (run 26685130999) ran full suite + `npm publish --provenance`. GitHub Release: <https://github.com/LH8PPL/claude-memory-kit/releases/tag/v0.1.2>. The friend now gets the fixed kit via `npm install -g @lh8ppl/claude-memory-kit`. All Phase-1 self-test bugs (#R/#0/#0b/#1/#3/#4) are now in users' hands.
2. ✅ **DONE — SonarCloud "Automatic Analysis" disabled** (2026-05-30); CI-based "SonarQube Cloud" scan is the sole analyzer. The stale "SonarCloud Code Analysis" red check on #79 was a dead context (disabled method, never re-runs) — #79 merged over it intentionally; real scan passed.
...
### v0.1.2 release status — MERGED, awaiting tag-push
- **PR #79 MERGED to main 2026-05-30** (`abcb118`) = version bump 0.1.1→0.1.2 + CHANGELOG [0.1.2] + install-message polish (outcome-over-inventory) + release-verification updated. Automatic Analysis disabled; merged over the stale Automatic red check (dead context).
- **Remaining: the tag-push** (Lior's outward-facing step) — `git tag v0.1.2 && git push origin v0.1.2` triggers `publish.yml` (OIDC + `npm publish --provenance`). That's the ONLY thing standing between main and npm serving the fixes.
