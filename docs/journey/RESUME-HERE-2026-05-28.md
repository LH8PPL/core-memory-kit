...
1. **Ship v0.1.2** — version bump + CHANGELOG + fixes are now **MERGED to main (PR #79, 2026-05-30)**. npm still serves v0.1.1 (buggy). **ONLY remaining step (Lior's, outward-facing):** `git checkout main && git pull && git tag v0.1.2 && git push origin v0.1.2` → `publish.yml` ships `@lh8ppl/claude-memory-kit@0.1.2` with provenance. After push: verify `npm view @lh8ppl/claude-memory-kit@0.1.2 version` + provenance badge.
