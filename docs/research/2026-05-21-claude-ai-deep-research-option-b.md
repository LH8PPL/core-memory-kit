...
This research output assumes a "regulated bank / air-gapped" deployment context throughout, despite the prompt being scoped to a personal open-source project. This is a known Claude.ai memory-injection artifact (the user works at a bank; Claude.ai's memory feature pulled that context into research mode unprompted).
