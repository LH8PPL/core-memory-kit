...
7. **Live verification at build time (binding "live-test every task"):**
   ...
   - **Highest-uncertainty live checks (flag for manual session — a one-shot CLI can't reach these):**
     ...
     - **Transcript format: ✅ RESOLVED 2026-06-20 (verified on a real Kiro install — no longer a live-check unknown).** Kiro is a **VS Code fork**; transcripts live at `%APPDATA%/Kiro/User/globalStorage/kiro.kiroagent/workspace-sessions/<base64url(workspacePath)>/`. Each project dir holds `sessions.json` (index: `[{sessionId, title, dateCreated, workspaceDirectory}]`) + one `<sessionId>.json` per chat, whose `history[]` array is the turn list — each turn = `{message: {role, content: [{type:'text', text}]}, contextItems, editorState}`. Structured JSON with role+content — close to Claude Code's shape but **JSON-per-session (not JSONL)** and keyed by a **base64url-encoded workspace path** (not the `~/.claude/projects/<slug>/` slug). The kit's hardcoded Claude-Code touchpoints (`env -u CLAUDECODE`, `~/.claude/projects/<slug>/<session>.jsonl`) just become 3 per-agent params: `transcriptDir`, `workspaceKey = base64url(workspacePath)`, `parse = JSON .history[].message{role,content[].text}`. Risk downgraded HIGH→LOW.
   - Record live results in the Task 50 DECISION-LOG entry; do NOT claim "verified" for the hook-firing layer until exercised live (transcript format IS now verified, above).
...
1. **~~Kiro transcript format is UNVERIFIED (highest risk).~~ ✅ RESOLVED 2026-06-20 — verified on a real Kiro install (the maintainer has Kiro installed; inspected directly).** Kiro is a VS Code fork; transcripts are per-session JSON at `%APPDATA%/Kiro/User/globalStorage/kiro.kiroagent/workspace-sessions/<base64url(workspacePath)>/<sessionId>.json` (+ a `sessions.json` index), each with a `history[]` of `{message:{role,content[].text}}` turns — clean, parseable, close to Claude Code's shape. Risk downgraded HIGH→LOW; the only port work is parameterizing 3 touchpoints (dir / base64url-workspace-key / JSON-`.history` parse). _The "unknown" was only "the docs don't document it"; the install answered it immediately — a reminder that primary-source verification includes inspecting the real artifact, not just the vendor docs._
