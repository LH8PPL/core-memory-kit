...
> **Implementation note (2026-06-18):** the Task 159 build diverged from this note's "Placement" section on two specifics — the fallback trigger (a standalone `isJournalStale()` boolean, not a `journal-stale` verdict) and the staleness signal (`INDEX.md` mtime, not "newest fact" mtime). Both recorded with rationale in **DECISION-LOG.md (2026-06-18 FIX+NOTE under D-169)** — the authoritative home for the pivot. Design §8.2.4 carries the composition reasoning.
