...
**CONFIRMED by the official [skill-creator](https://github.com/anthropics/skills/tree/main/skills/skill-creator) SKILL.md (2026-06-03):** its OWN frontmatter is exactly `name` + `description` (no other fields); it states verbatim *"All 'when to use' info goes here [description], not in the body."*; body <500 lines (add a hierarchy layer + reference files past that); optional bundled dirs `scripts/` (executable), `references/` (loaded as needed), `assets/` (output files); reference files >300 lines get a TOC. **NOTE on the two doc sources (the user's Q):** `platform.claude.com/.../agent-skills/*` (best-practices, the authoring source) ≠ `code.claude.com/docs/en/skills` (the Claude Code *product* doc — owns the `allowed-tools` field + `.claude/skills/` install location + model-invocation). They overlap on the core; use the platform docs for authoring quality and the Claude Code doc for the runtime `allowed-tools` field. Both confirm: when-to-use in `description`, <500 lines, progressive disclosure.

**Refined 69.0 spec (FINAL — supersedes the bullets above):** `name: memory-write` (or `capturing-memory`); `description:` third-person what+when (triggers embedded, NOT a separate `when_to_use` field); `allowed-tools: Bash(cmk*) Read` (Claude Code field); body = LOW-freedom `cmk remember "<fact>" --why --how --type` (+ remove/replace verbs), a **MUST/NEVER hard gate** ("NEVER hand-edit `context/memory/`"), forward-slashes, NO dev-repo paths, <500 lines, concise (assume Claude knows the basics). **BEFORE writing 69.0 — READ the actual product SKILL.md files (primary source, not this summary; the user 2026-06-03 caught that I was relying on the survey):** `/c/tmp/skill-research/gstack/learn/SKILL.md` (**closest analog** — model the rewrite on it) · `/c/tmp/skill-research/gstack/context-save/SKILL.md` (hard-gate) · `/c/tmp/skill-research/memsearch/plugins/claude-code/skills/memory-recall/SKILL.md` (read-only recall via Bash CLI) · `/c/tmp/skill-research/claude-mem/plugin/skills/mem-search/SKILL.md`. Read top-to-bottom for concrete structure/wording, THEN author — not from the summary. Baseline for eval-driven iteration = the existing `npm run live-verify` runs.
