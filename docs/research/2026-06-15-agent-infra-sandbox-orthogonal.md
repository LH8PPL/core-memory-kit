...
## On containers (the kit does NOT go near Docker — the user 2026-06-15: "i dont like the docker")

To be unambiguous: **the kit never ships, requires, provisions, or targets Docker / containers / a sandbox.** That's D-23 (node-only, no-server) + the local-first ethos. agent-infra/sandbox is the runtime an agent might run in; the kit is memory that rides inside whatever environment the agent already has — it stays out of the execution-environment business entirely. No "containerized-agent support" is on any lane, and none should be added on the strength of this note. The ONLY container-adjacent fact worth a passing mention (NOT a task, NOT a goal): the kit is plain files, so it is incidentally container-agnostic — if someone else's harness happens to be a container, the committed `context/` survives as long as the repo is the mounted/persisted volume; that's the host's concern, not the kit's. End of the container topic.
