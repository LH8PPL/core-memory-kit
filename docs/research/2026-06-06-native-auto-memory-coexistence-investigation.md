...
4. A CLAUDE.md line steering off native is cheap belt-and-suspenders but, per Anthropic's own docs, **won't reliably hold** — so it cannot be the primary fix. A **PreToolUse hook** is the only *hard* block (v0.3 option for strict-authoritative users).
