...
- [~] **94 — load-cap (not write-cap) + universal graduation** — _CORE shipped 2026-06-04, PR #114 (D-61/D-63): 94.1 load-cap (writes always succeed; no write-lock on any tier) + 94.2 all-tier graduation (persona graduates to `fragments/` — the D-60 persona-lock fix). The never-lose invariant is delivered. Follow-ups: 94.3 (proactive SessionEnd pass), 94.4 (= Task 93), 94.5 (live cold-open re-test)._
