...
- [ ] **122** — 🔴 name-privacy: extend the D-51 de-id sweep to CODE (the maintainer's real name is in 4 shipped `src/` files + 5 scripts; pushing the tag publishes it to npm) + add a guard so it can't recur.
- [ ] **121 (user-facing parts)** — fix the MCP server's hardcoded `version: '0.1.0'`, the six "v0.1.0/v0.1.x" user-facing capture messages, CLI.md's stale 5-tool list, and flip ADR-0014 → Accepted. (Don't ship "v0.1.0" in the v0.2.3 headline surface.) Internals (parity-guard scope, comment normalization) trail to v0.2.4.

**→ v0.2.4 — housekeeping (the next patch after v0.2.3).** Cleanup surfaced during the v0.2.3 cut + pre-existing CI hygiene; no new differentiator (still wedge polish).

- [ ] **120** — remove the stale `memsearch + Milvus` scaffolding (`doctor` HC-1/HC-7 + the shipped `milvus-deploy`/`memsearch-index` template files + genericize the user-facing "memsearch" strings). **KEEP** the `semanticBackend` seam — the §9.3.1 backend-choice deferral STANDS; this only removes the OLD candidate's premature scaffolding. Surfaced 2026-06-08 when `cmk doctor` showed "memsearch installed" during the v0.2.3 cut (D-101). _Detail in [`tasks.md`](../specs/tasks.md) Task 120._
- [ ] **119** — bump Node-20 GitHub Actions ahead of the runner deprecation (back-compatible until Sept 2026; do before the publish path breaks).
