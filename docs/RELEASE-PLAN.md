...
- **v0.3.2** — **141b: the `node:sqlite` migration** (the structural npm-12 cure — zero native deps, immune to the install-script class) + the Node-engines-floor bump (≥22.5, the user unblocked it 2026-06-12) + whatever else closes the v0.3.x lane (96/130 if ready). **GATED on two spikes that BOTH must pass before migrating (D-147):**
  1. **Perf bake-off** (`scripts/bench-storage.mjs`): time `node:sqlite` vs `better-sqlite3` on the kit's real hot paths (reindexFull bulk insert · per-read incremental reindex · FTS5 keyword query · sqlite-vec cosine query · single `cmk remember` write), p50/p95 both backends same corpus. **Bar: node:sqlite within a small p95 regression budget on the READ/search paths** (the user-visible latency — propose ≤10-15%, the user sets the final number); write/bulk have more slack. The D-109 embedder-bake-off discipline, applied to storage — *"i dont want to give people a slow response just because node:sqlite is easier on the npm v12 problem"* (the user). **If it fails the bar, 141b does NOT ship — better-sqlite3 stays, 141a's install-time ask remains the npm-12 answer.**
