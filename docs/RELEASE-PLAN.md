...
- [x] **91 — MEMORY.md graduation (write-lock safety valve)** — _shipped 2026-06-04, PR #113 (D-54/D-57). Transactional cap-pressure valve, project MEMORY.md only; recall of graduated facts is v0.3 (Task 75)._
- [ ] **94 — load-cap (not write-cap) + universal graduation** — _BINDING architecture, D-61; design §19. Generalizes Task 91 to ALL tiers + removes the write-reject (writes always succeed; cap = inject budget only; overflow graduates to the searchable store). Fixes the persona write-lock (D-60/cut-gate2 §6). The invariant: never lose memory. This is the v0.2.0-completing piece — design-first (done), then TDD._
