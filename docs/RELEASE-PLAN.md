...
- **v0.3.2** — **153: FTS5 query sanitization** (dots/version strings crash `mk_search` — surfaced 2026-06-15) + **152: `validate-index-completeness`** (dev-tooling; converts the recurring index-lag drift class into a prerun check) + **147: `cmk digest` + standing `context/DECISIONS.md`** (the decision-journal the user called for) + **141b: the `node:sqlite` migration** (the structural npm-12 cure — zero native deps, immune to the install-script class) + the Node-engines-floor bump (≥22.5, the user unblocked it 2026-06-12) + whatever else closes the v0.3.x lane (96/130 if ready). **GATED on two spikes that BOTH must pass before migrating (D-147):**
