...
- **v0.3.2 (FINAL SCOPE, D-162)** — **153: FTS5 query sanitization** ✅ + **152: `validate-index-completeness`** ✅ + **147: `cmk digest` + standing `context/DECISIONS.md`** ✅. **141b (`node:sqlite` migration) REJECTED on perf** (2026-06-16, D-162): clean CI bench showed node:sqlite ~10% slower on FTS5 keyword search — better-sqlite3 stays, 141a is the npm-12 answer. v0.3.2 is ready to cut. _(Original 141b plan below preserved for the decision trail.)_
...
- **v0.4.1 — Cursor** (Task 50, locked).
- **Then, an ORDERED tail — each assigned its patch number WHEN IT SHIPS** (not pre-numbered; the patch digit tracks shipments, and the order may shift on real signal — a Codex API change, a Cursor breaking release, the user's usage): **Codex → Google Antigravity → gemini-cli → opencode → cline/roo/windsurf/… as demand warrants.** (Prior-art profiles for several of these already exist in Taskmaster — design input, not a port.)
...
- **v0.4 curation lane:**
  - **151** — persona-promotion redesign: replace `PERSONA_CONFIDENCE_RULE`'s form/phrasing gate with a recurrence-scored, LLM-judged promotion pass; drop the human-review-queue (post-hoc `cmk forget` reversibility). Direction decided + down-payment shipped in v0.3.1 (D-154); this is the full redesign. _Deep research → design doc → implementation._
