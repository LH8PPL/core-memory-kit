...
| --- | --- | --- |
| 1. Structural buffer cap | MemoryOS `deque(maxlen)`, MemOS `MAX_BUFFER_SIZE=10MB`, Letta `message_buffer_limit=60`, caura-memclaw `MAX_INGEST_WRITES_PER_SESSION=10` | **Yes** — a `now.md` byte/turn cap (mechanism B below) |
| 2. Deterministic pre-truncate/chunk before the model | memsearch `MAX_PROMPT_CHARS=80_000`, mempalace `CHUNK_SIZE`/`TruncationDetected`, graphiti `truncate_at_sentence`, TencentDB emergency `truncate-in-place` | **Yes** — the input cap + keep-last-N pre-trim (mechanism A) |
| 3. Partial / windowed compaction (compress oldest, keep recent verbatim) | Letta `PARTIAL_EVICT` 30/70, OpenHands `keep_first=2`, langmem/LightMem `get_dialated_windows(N=5)`, squad inbox-deltas | **Yes** — partial-evict (mechanism C) |
| 4. Batch-size cap (≤N units per pass) | TencentDB `Max tool pairs per L1 batch = 20` | folds into mechanism B (cap the per-call input) |

**The adopted mechanism set** (sized in bytes/turns against measured Haiku latency, NOT tokens-against-a-context-window — `now.md` is compressed offline, never fed to a live inference, so the field's token-pressure *trigger* does not apply; only their input-bounding *mechanics* do):

- **A. A hard `now.md` input cap at the compress call (families 2+4) — the primary, highest-certainty fix.** Before `backend.compress()`, if the buffer exceeds `COMPRESS_INPUT_CAP_BYTES`, deterministically reduce it (keep the most-recent `KEEP_LAST_TURNS` verbatim, hand only the older remainder to Haiku — never the whole thing). Zero added latency, no LLM, breaks the growth→latency coupling at the call site. `input_bytes` is already measured — gate on it. The cap is sized so the worst-case input stays well under the latency that produces a 50s timeout (the dogfood data sets the ceiling: inputs ≥166 KB timed out, so the cap lands well below that — exact value pinned with the implementation against fresh latency measurement).
- **B. A `now.md` size/turn cap that triggers mid-session compaction (family 1) — drain while small.** Rather than wait for SessionStart to roll a huge backlog, when `now.md` crosses the cap mid-session (a cheap byte check at the §16.27 PostToolUse append site), trigger a bounded compaction of the overflow. This makes the spiral *structurally impossible* rather than merely survivable. (Composes with the existing append path; the check is O(1).)
- **C. Partial-evict compaction (family 3).** Even when A trims, structure the trim as "summarize the OLDEST slice, KEEP recent N turns verbatim," snapped to a turn boundary (`now.md` has turn delimiters; Letta snaps to an assistant message). Decouples the compaction input from total session length and preserves recent context the summary would otherwise blur.
- **D. Defense-in-depth: `minimum_progress` floor + shrink-and-retry on `haiku_timeout` (OpenHands `minimum_progress=0.1`, `context_scaling=0.8`×5).** On a timeout, RETRY with a deterministically-shrunk input (drop the oldest slice, re-call) so a slow call degrades gracefully instead of spiraling; skip a Haiku call that would shrink <~10%. Task 84 already noted shrink-and-retry; this confirms it from the survey and pairs it with the min-progress guard.

**Where it fires.** Keep the SessionStart-lazy roll (§8.2.2), and ADD a **bounded SessionEnd drain** (the user's hunch + squad's session-end ceremony + Letta's eviction-on-step) so the buffer is kept small at the freshest boundary. **Composition constraint (§8.5 / D-42):** SessionEnd already runs `compressSession` CONCURRENTLY with `autoPersona` under the 60s ceiling — so the SessionEnd drain must be a *bounded* compaction (mechanisms A–C keep its input small ⇒ fast), NOT a second full-buffer 50s call. The bounded input is precisely what makes a SessionEnd roll safe under the ceiling.

**Family coverage (binding — cut-gate-16 §7 F-4).** `daily-distill` and `weekly-curate` route through the same `HaikuViaAnthropicApi.compress` under the same budget; their inputs are summaries-of-summaries, so the input cap (A) + partial-evict (C) generalize cleanly (per-source cap + oldest-first). The fix is applied to all three lifecycle compression verbs, not just the now→today roll.
...
**Cross-references:** §8.5 (timeout/hook-ceiling composition — the budget this composes with), §8.2.2 (lazy-compress — the SessionStart roll), §8.1 (SessionEnd pipeline + the D-42 concurrent budget), §16.27 (PostToolUse → `now.md` append — the mid-session cap site), §8.3 (`CompressorBackend` — where the input bound lives), Task 84 (compressor grounding/supersede + the OpenHands shrink-retry note), Task 105 / D-75 (the unbounded-`now.md` class this re-instances). _Implements: Task 161; the [19-system research note](../docs/research/2026-06-18-session-buffer-compaction-under-latency-growth.md); D-173._
