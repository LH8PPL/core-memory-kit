...
**It bit on its first real run:** 62 raw hits → 7 after the three false-positive classes above → **1 genuine drift** — `specs/glossary.md` still described `cmk doctor` as running "all 7 health checks (HC-1..HC-7)". Twelve exist. That line had survived five minor releases and every prior doc review. <!-- validate-docs: ignore -->
