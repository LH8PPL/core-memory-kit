...
- **Built (151.5) — the sweep order is value-ordered on BOTH surfaces, a two-axis CONJUNCTION (never single-axis — single-axis IS the MemoryOS-LFU/MemOS-top-N bug):**
  ...
  - **⚠️ 151.6 re-evaluation (binding):** both surfaces key on the `trust` ENUM (high/medium/low) + capture-`at` as the "long-unaccessed" proxy — the only signals a scratchpad bullet carries today (no last-access field exists on bullets). When **151.6** lands the real `trust_score` FLOAT (in the rebuildable index) + a genuine access/recurrence signal, the sweep is upgraded to rank on those (a continuous score + true last-access beats the 3-bucket enum + creation-age proxy). The CONJUNCTION shape is invariant across the upgrade; 151.6 only raises the fidelity of each axis. See the `// 151.6: re-evaluate` markers at both code sites.
