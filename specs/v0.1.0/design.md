...
| --- | --- | --- |
| §17.1 — every test file declares its exit-door coverage via `// @doors:` header (and explicitly marks N/A doors with reasons) | [`scripts/validate-exit-doors.mjs`](../../scripts/validate-exit-doors.mjs) | **Warning** during PR-D2b annotation pass (header-missing files warn but do not fail the suite). **Strict** under `CMK_DOORS_STRICT=1`; flipped to default at PR-D2b's strict-mode-flip sub-task once all 30 test files are annotated. Silent-omission (declared neither in-set nor N/A) violates regardless. |
