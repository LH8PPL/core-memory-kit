...
- [ ] 91. **MEMORY.md graduation — the missing 3rd shrink mechanism (the hot index write-locks without it).** _BLOCKER, surfaced by the 2026-06-04 cut-gate run; D-54. Scope LOCKED to Decision A (D-57): a minimal SAFETY VALVE — search-only graduation, project MEMORY.md only, fires only at cap pressure to prevent the write-lock. Recall of graduated (search-only) facts is v0.3 (Task 75); v0.2.0 accepts that overflow facts are searchable-not-auto-injected. This is the SOLE remaining hard v0.2.0 blocker._ Design intent (THREE sources — [design.md §987](../../specs/v0.1.0/design.md), [2026-06-01 lifecycle research](../../docs/research/2026-06-01-memory-lifecycle-and-competitive-position.md) lines 56+60, chatgpt-design.md): MEMORY.md is a small HOT INDEX bounded by **(1) byte cap + (2) stale-drop >14d + (3) GRADUATION — durable facts move OUT into `context/memory/*.md` fact files**. Code ships (1)+(2) only; **graduation is unbuilt** (`grep graduat` → 0 source hits; `consolidate()` only hard-deletes low/medium >14d; `daily-distill` is sessions-only). Consequence (live-measured): cut-gate MEMORY.md = 2059/2500 B after ONE session, all 6 bullets `trust: high` → `consolidate()` frees nothing → ~1 more session → `CAP_EXCEEDED` → kit silently stops remembering. Also subsumes the cut-gate **double-capture** (graduation moves the auto-extract bullet INTO a fact file and dedups against an existing `cmk remember` file).
  - [ ] 91.1 **Graduate high-trust MEMORY.md bullets → granular `context/memory/*.md` fact files**, content-addressed, deduped against existing facts (cross-store sha1-of-canonical-text check across BOTH the scratchpad AND the fact-file index before write). Removes the bullet from MEMORY.md on successful graduation (the drain) and leaves a pointer/INDEX entry so recall still resolves.
  ...
  - [ ] 91.3 **Trigger wiring.** Run graduation where it composes safely (candidate: the SessionEnd pass alongside compress/auto-persona, design §6.8/§7.1 — disjoint inputs from compressSession). Must keep MEMORY.md under the consolidation trigger after a busy session.
