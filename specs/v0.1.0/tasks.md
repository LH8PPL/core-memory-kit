...
- [ ] 53. Package security hardening (T-041) — **MUST happen (Lior 2026-05-29: "we need to do a security task… it needs to happen")**
...
- [ ] 53.1 **Secret scanning** — wire gitleaks (or GitHub secret scanning) into CI on push + PR. Highest priority: catches both a leaked token AND a secret that auto-extract wrote into `context/`. Add a `scripts/validate-no-secrets.mjs` pre-test gate if a lightweight local check is wanted too.
- [ ] 53.2 **Supply-chain** — `npm audit` (fail on high/critical) in CI + enable Dependabot for the 7 runtime deps (better-sqlite3, chokidar, commander, js-yaml, zod, @modelcontextprotocol/sdk, @lh8ppl/cmk-canonicalize). Commit a lockfile if not already pinned.
- [ ] 53.3 **Publish provenance** — add `--provenance` to the npm publish flow (free signed attestation since we publish from GitHub; proves the tarball came from our repo + workflow). Document in the publish playbook.
- [ ] 53.4 (optional) **SAST** — evaluate CodeQL on the JS; likely overkill for a kit this size — decide after 53.1–53.3 land. Document the keep/skip call either way (decision-trail).
- [ ] 53.5 **Threat-model doc** — short `docs/SECURITY.md`: the kit's threat surfaces (hook execution, subprocess spawn, auto-extract-into-committed-files, memory poisoning), what's mitigated where (poison-guard, `<private>`, no-silent-network NFR-5), and the responsible-disclosure contact. Also add the npm `bugs`/security policy.
- [ ]* 53.6 Tests / CI — the CI workflow(s) run green on all 3 OSes; secret-scan + audit gates actually fail a seeded-bad PR (prove the gate bites, not just that it runs).
