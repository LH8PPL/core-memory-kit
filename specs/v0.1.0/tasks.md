...
- **Also done 2026-05-31:** ADR-0011 resolved (#89) — coexist with Anthropic's native auto-memory by default + `cmk disable-native-memory` as a committable opt-in (D-21). Two new tasks specced, **NOT built:** **Task 60** (the disable command + install/doctor wiring) and **`Task 61`** (inline cross-project auto-persona — spec committed on branch `task-61-inline-persona`, not yet merged).

- **▶ THE PLAN (agreed 2026-05-31) — ship v0.2.0 to the friend soon. Order:**
  1. **Build `Task 61` FIRST** (inline cross-project promotion). Auto-persona currently fires only **weekly**, so the cross-project differentiator ("knows my prefs in every project") is invisible in a short trial — re-opening the `#2` user-tier-empty gap in practice. Lior: *"nobody is going to wait a week, not even a day."* `Task 61` promotes cross-project facts **at capture time** (Layer 1); the weekly pass becomes a dedup janitor (Layer 2). **This is what makes v0.2.0 worth shipping.** Spec ready on branch `task-61-inline-persona`; touches core `auto-extract` → build **TDD-first + 5× stress + two-pass review**.
  2. **Fresh live test** — run the `docs/process/` scripts against **local `main`** (NOT npm — npm is still v0.1.2; main has the unreleased Phase 2). This is the **v0.2.0 release gate**. Claude drives the automatable half (sandbox install from the repo + `--plugin-dir` + CLI); Lior drives the fresh-session recall half.
...
- [ ] 45. Auto-persona generation (T-014) — **v0.2 Phase 2; the friend-handoff gate (self-test finding #2, 2026-05-30)** _core (automatic path) shipped 2026-05-30, PR #83 — parent stays open for two follow-ups: manual `cmk persona generate` wrapper + low/medium-confidence → review-queue FILE write (see 45.6)._
...
- [ ] 62. Node-only hook execution — retire the bash dependency (cross-OS parity with Claude Code)
  ...
  - [ ] 62.1 **PRIMARY-SOURCE CHECK FIRST**: verify against Anthropic's plugin-hooks docs whether Claude Code expands `${CLAUDE_PLUGIN_ROOT}` itself (before invoking the shell) or relies on the shell to expand it. If Claude Code expands it, the plugin `hooks.json` command can become `node "${CLAUDE_PLUGIN_ROOT}/bin/<name>.mjs"` (shell-agnostic). If it relies on the shell, document the constraint + decide (keep bash for the plugin route only, or use a node launcher). Record in design §5.1 + an ADR.
  - [ ] 62.2 Convert the hook-bin tests (`cli-hooks-scaffold`, `cli-capture-prompt`, `cli-capture-turn` bash-wrapper describes, `cli-observe-edit`) from `spawnSync('bash', [stub])` to `spawnSync(process.execPath, [mjsBin])` — testing the bins the way the npm route actually runs them. Kills the ambient-bash dependency on every dev/CI machine (this is the self-contained, highest-value piece — do it even if 62.1 keeps bash for the plugin route).
  - [ ] 62.3 Per 62.1's finding: update `plugin/hooks/hooks.json` to the node form (or document why bash stays); retire the extensionless bash wrappers in `plugin/bin/` + the legacy `auto-extract-memory.sh` (design §18 cleanup).
  - [ ] 62.4 Update the shebang-mode test (`begins with a bash/sh shebang`) to assert the node shebang on the `.mjs` bins; README/INSTALL note that the kit needs only node (no bash) on all 3 OSes.
