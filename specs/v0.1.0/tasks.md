...
- [~] 94. **Memory-retention architecture — load-cap (not write-cap) + universal graduation.** _CORE shipped 2026-06-04, PR #114 (94.1 load-cap + 94.2 all-tier graduation — the write-lock + persona-lock fix; D-63). Remaining: 94.3 (proactive SessionEnd pass — optimization), 94.4 (= Task 93), 94.5 (live cold-open re-test)._ _BINDING architecture, D-61; design [§19](design.md). Supersedes the write-cap behavior + the D-57 "defer user-tier graduation" call; generalizes Task 91 to all tiers._ The invariant: **memory is never lost; always retrievable** (the kit's reason to exist). Diagnosed from cut-gate2 §6 (D-60): the persona write-locked (`not-promoted-cap_exceeded`) because the user tier has the cap but no graduation valve. Validated against Anthropic's own model (load first 25KB + on-demand topic files — D-60). **Storage half (94.1-94.3) is deterministic + satisfies the no-loss invariant alone; the retrieval-reliability half is Task 75 (v0.3).**
  ...
  - [ ] 94.3 **Periodic graduation pass at SessionEnd** so each scratchpad's injected slice stays under its load-cap (compose with session-end-tasks.mjs, disjoint inputs per §6.8/§7.1).
