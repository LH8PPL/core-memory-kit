...
- [x] 86. **WEDGE BUG — multi-rule turns don't promote to the cross-project persona (lior-test-8 finding, 2026-06-03).** The auto-promote (Task 78) promotes a STATED universal rule to the user tier at `trust: high`. But it only fires when the turn carries **one** persona candidate: `extract.log` shows `obs=1 → persona_promoted:1` (venv, structure, gitignore each promoted alone) yet **`obs=3 → persona_promoted:0`** (the turns bundling type-hints+TDD, and uv+ruff+secrets, promoted NOTHING — their observations routed to `queues/review.md` at medium trust instead). Result: the user tier captured **3 of 8** stated universal rules; **uv, ruff, secrets-in-.env, type-hints, TDD are missing from the persona**. **Impact: the cross-project cold-open** (the wedge's whole pitch, D-27) — a NEW project would only inherit the 3 promoted rules. Within the originating project all 8 still recall from the project fact files, so this is invisible until the cold-open. **Likely cause:** the inline auto-promote path in [auto-extract.mjs](../../packages/cli/src/auto-extract.mjs) handles a single persona candidate per turn and drops the rest (loop/array bug). Fix: promote ALL high-confidence cross-project candidates the turn emits, not just the first. Add a test seeding a multi-rule turn → assert every stated universal rule lands in the user tier. **FIXED 2026-06-03 (branch task-86-multi-rule-promote):** root cause confirmed NOT a code bug — both parsePersonaCandidates and promoteCandidatesToUserTier correctly loop over all candidates; Haiku was DROPPING the trailing singular "ALSO — emit one ADDITIONAL line" addendum under load (busy multi-rule turn). Fix is prompt-side: the persona-candidate instruction is now REQUIRED + PER-FACT ("three rules → three lines, never collapse, never skip"). LIVE-VERIFIED against real Haiku: a 3-rule turn that previously promoted 0 now emits 3/3 persona candidates across 3/3 runs. Prompt-content test added; full suite 1350. _Requirements: the wedge (D-27); design §16.16 (auto-persona); surfaced by lior-test-8; relates Task 78._
