...
- [ ] 77. **Rename version-named-but-general docs → version-agnostic — the misnomer class (the user 2026-06-02 + 2026-06-04).** Two version-named things that are actually project-general and now lie about their scope:
  - **`specs/v0.1.0/` → `specs/`** — `specs/v0.1.0/{requirements,design,tasks,glossary}.md` is the **living spine** (per D-19, single evolving source of truth) — it holds Tasks 63–76, design §16.19, the v0.2/0.3 roadmap, etc. — all FAR past v0.1.0. The folder name is a historical artifact from when v0.1.0 was the only version. Flatten to `specs/*.md` (or `specs/current/`).
  - **`docs/journey/v0.1.0-build-log.md` → `build-log.md`** (added 2026-06-04, the user: *"why are you writing to a file called v0.1.0 when we're at v0.2?"*) — it's the single ongoing **narrative + per-PR retrospectives** log (CLAUDE.md's source-of-truth table points all narrative here); it already holds v0.2 retros (Tasks 45/61/76/78/69). Same misnomer: the name says v0.1.0, the content is project-general. _(If it grows unwieldy later, split by version with an index — but the immediate fix is the version-agnostic name, not a split.)_

  Both are mechanical-but-wide: `validate-references` enforces every internal link, so ALL references (across docs/, README, CLAUDE.md's source-of-truth table + narrative pointers, DOCUMENTATION-MAP, the validators' path constants) must update in lockstep. Do as **one atomic refactor PR** covering both renames. _Requirements: D-19 (living single-source spine); naming hygiene._
