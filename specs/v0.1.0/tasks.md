...
- **▶ ACTIVE — documentation-governance restructure.** Establishing the 3-zone spine + a registry validator and collapsing the ~7 overlapping state surfaces into the spine, so consistency survives a context-compact (structural, not memory). Plan + rationale: [`docs/DOCUMENTATION-MAP.md`](../../docs/DOCUMENTATION-MAP.md). Increments: (1) harness — map + `validate-doc-registry.mjs` ✓ · (2) fold PHASE-3-PLAN + this state section ✓ · (3) merge `requirements-revisions-proposed.md` into `requirements.md` · (4) demote RESUME-HERE/BOOTSTRAP to pointers + refresh stale indexes.
