...
- [ ] 92. **LOW-trust drops must leave a trace (G6 from the lifecycle map).** _v0.2.x (not a cut blocker); D-55._ Auto-extract LOW-trust candidates are **discarded with no record of the content** ([auto-extract.mjs](../../packages/cli/src/auto-extract.mjs) — `low → discarded`, logged only as a `skipped_reason: nothing_durable` COUNT). A genuinely-useful fact Haiku mis-graded LOW (or an assistant-origin fact demoted to LOW) vanishes from active memory with no auditable trace — the §6.5 "don't lose without trace" principle applied to the *capture* edge (MEDIUM gets a review queue; LOW gets nothing). Partial mitigation today: the verbatim turn survives in `transcripts/{date}.md`, but it's not auto-re-mined and you can't tell *what auto-extract chose to drop*.
  - [ ] 92.1 Log each discarded LOW candidate's **content excerpt + reason** to `extract.log` (NDJSON, Door-4) so the drop is auditable/recoverable without polluting active memory. (Decision: log-only, NOT route LOW to the review queue — that floods it with noise.)
  - [ ] 92.2* Test: a LOW candidate produces an `extract.log` entry carrying the excerpt (not just a count); over-mutation — a HIGH candidate in the same turn still lands in MEMORY.md.
  - _Relates G6, §6.5 (trace principle), Task 91.2 (the eviction-edge analog). Filed 2026-06-04._
