...
- [ ] 93. **Inject drop must be importance-aware, not tail-order (G7 from the lifecycle map).** _v0.2.x (not a cut blocker; bites harder once tiers fill); D-55._ When a tier exceeds its inject budget, `truncateTierToBudget` ([inject-context.mjs](../../packages/cli/src/inject-context.mjs)) drops whole `## ` sections **from the tail** — so which real facts reach the context window depends on file/section ORDER, not trust or recency. A high-value fact in a late section (e.g. `Pending Decisions`) can be dropped while a lower-value earlier section survives. It IS logged (`tier_truncated` NDJSON), so observable — but the selection is wrong. Composes with G1: post-graduation the P tier stays small, but the USER tier (the wedge's persona payload across USER/HABITS/LESSONS, 4975B budget) can still overflow and tail-drop persona sections the cold-open needs.
  - [ ] 93.1 Replace tail-drop with **value-ordered eviction**: when over budget, drop lowest-trust / oldest bullets (or whole sections by aggregate value) first; never drop a high-trust bullet before a low-trust one (mirror `consolidate()`'s high-trust protection at the inject layer).
  - [ ] 93.2* Test: a tier over budget with a high-trust bullet in the LAST section + a low-trust bullet in the FIRST → the low-trust one is dropped, the high-trust one is injected (the inverse of today's tail-order behavior). Door-4: the `tier_truncated` event records which bullets were dropped + why.
  - _Relates G7, §7.1 (per-tier budgets), the wedge cold-open (US-2). Filed 2026-06-04._
- [~] 94. **Memory-retention architecture — load-cap (not write-cap) + universal graduation.** _CORE shipped 2026-06-04, PR #114 (94.1 load-cap + 94.2 all-tier graduation — the write-lock + persona-lock fix; D-63). 94.3 (proactive SessionEnd sweep) shipped 2026-06-05 (D-65). Remaining: 94.4 (= Task 93 importance-aware inject), 94.5 (live cold-open re-test)._ _BINDING architecture, D-61; design [§19](design.md). Supersedes the write-cap behavior + the D-57 "defer user-tier graduation" call; generalizes Task 91 to all tiers._ The invariant: **memory is never lost; always retrievable** (the kit's reason to exist). Diagnosed from cut-gate2 §6 (D-60): the persona write-locked (`not-promoted-cap_exceeded`) because the user tier has the cap but no graduation valve. Validated against Anthropic's own model (load first 25KB + on-demand topic files — D-60). **Storage half (94.1-94.3) is deterministic + satisfies the no-loss invariant alone; the retrieval-reliability half is Task 75 (v0.3).**
  ...
  - [ ] 94.4 **Importance-aware inject = Task 93 (G7).** The inject step selects the highest-value slice (trust/recency) and graduates the long tail — not tail-order. Build 93 as the inject-selection half of this architecture (compose, don't duplicate).
