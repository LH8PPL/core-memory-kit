...
- [ ] 94. **Memory-retention architecture — load-cap (not write-cap) + universal graduation.** _BINDING architecture, D-61; design [§19](design.md). Supersedes the write-cap behavior + the D-57 "defer user-tier graduation" call; generalizes Task 91 to all tiers._ The invariant: **memory is never lost; always retrievable** (the kit's reason to exist). Diagnosed from cut-gate2 §6 (D-60): the persona write-locked (`not-promoted-cap_exceeded`) because the user tier has the cap but no graduation valve. Validated against Anthropic's own model (load first 25KB + on-demand topic files — D-60). **Storage half (94.1-94.3) is deterministic + satisfies the no-loss invariant alone; the retrieval-reliability half is Task 75 (v0.3).**
  - [ ] 94.1 **Remove the write-cap reject — writes ALWAYS succeed.** Delete the `cap_exceeded` error path in `appendScratchpadBullet`; scratchpad files grow on disk; the per-file caps become **inject (load) caps only** (design §19.1). No write-lock on any tier.
  - [ ] 94.2 **Generalize graduation to ALL tiers.** Lift the `tier==='P' && scratchpad==='MEMORY.md'` gate (Task 91): the user-tier persona (USER/HABITS/LESSONS) + SOUL.md graduate their overflow too. The **user tier gains a fact store** (parallel to `context/memory/`) for graduated persona content, searchable via `cmk search`/`mk_get`. Keep the content-id cross-store dedup + transactional rollback from Task 91.
