...
- [ ] 61. Inline cross-project promotion — auto-persona fires *now*, not weekly (the "TikTok-age" responsiveness fix)
  ...
  - [ ] 61.1 Extract a shared `promoteCandidatesToUserTier()` helper from `auto-persona.mjs` (the promotion block) so both `autoPersona` (janitor) and `auto-extract` (inline) call ONE promotion path. Export `buildClassifierInstructions` + the `PERSONA_CANDIDATE_RE` parser for reuse.
  - [ ] 61.2 `auto-extract` prompt: append the cross-project `PERSONA CANDIDATE` directive (reuse `buildClassifierInstructions`'s body). Parse the persona lines alongside `parseCandidates`.
  - [ ] 61.3 Thread `userDir` into `runAutoExtract` + the `cmk-auto-extract` bin (resolve via `tier-paths` / `MEMORY_KIT_USER_DIR`); route persona candidates to the user tier when `userDir` is available (skip gracefully on a fresh machine with no user tier, like `autoPersona` does).
  - [ ]* 61.4 Tests — a turn with a cross-project preference → the fact lands in the **user tier** (HABITS/LESSONS/USER) **this run**, while project-specific facts still land in project `MEMORY.md`; over-mutation guard (project facts untouched; existing user-tier facts untouched); auto-supersede on conflict; trust:high hand-curated entry preserved. Doors 1 (return: promoted count) + 2 (user-tier scratchpad state) + 3 (the single Haiku call still has the documented shape — NO second call) + 4 (extract.log records the inline promotion). **Stress 5× (spawn/detached surface).**
