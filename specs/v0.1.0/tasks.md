...
- [ ] 69. **Skills as the delivery mechanism — both install routes equivalent; stop bloating the user's CLAUDE.md (D-28).** _PULLED INTO v0.2.0 2026-06-03 (D-48 has the full trace): the CLAUDE.md clutter is real + user-facing AND 69.0 is a SECURITY fix (the current skill hand-edits MEMORY.md, bypassing Poison_Guard = F1 leak class). The "test-first-then-move" precondition is MET (wedge live-verified with current CLAUDE.md). Needs a live-verify re-check after (changes the delivery layer)._ The npm route (`cmk install`) lost skill-scaffolding (regressed at Task 49) + substitutes a ~60-line memory-write procedure appended to the user's own `CLAUDE.md`; the plugin route ships the live skill. Anthropic's own docs say procedures belong in skills (load on-demand) and `CLAUDE.md` should stay <200 lines of *facts* — and the user's `CLAUDE.md` is theirs. Full what/why/when/where/how in [DECISION-LOG D-28](../../docs/journey/DECISION-LOG.md).
  - [ ] 69.0 **REWRITE the memory-write skill first — it's stale + unsafe (do NOT scaffold as-is).** The current `plugin/skills/memory-write/SKILL.md` grants `allowed-tools: Read Edit Write` and its routing table tells Claude to **hand-edit** MEMORY.md/USER.md → **bypasses Poison_Guard + home-path sanitization = the exact F1/v0.1.2 leak class** (it even falsely claims "runs through Poison_Guard"; with no `Bash` it can't invoke `cmk`). It also references dev-repo internals (`packages/cli/src/…`) a user doesn't have, and predates the "route everything through `cmk remember`" decision. Rewrite: `allowed-tools: Bash(cmk*)` (or `Read` + `Bash`), instruct Claude to capture via **`cmk remember --why/--how`** (rich, safe path) — never hand-edit `context/memory/`; drop dev-repo paths; keep the Anthropic-conformant frontmatter (`description` + `when_to_use`). _(Provenance: our Task-24 authored synthesis of Hermes add/replace/remove + claude-remember phrase-triggers; frontmatter per the skills doc — but the body needs the F1 alignment.)_
  - [ ] 69.1 `cmk install` (install.mjs) scaffolds skills into `<project>/.claude/skills/` (project-tier, travels with `git clone`) — restores what the original `install.sh` did; both routes now deliver the (rewritten, safe) memory-write skill as a real Claude Code skill.
  - [ ] 69.2 **One source of truth per skill** — eliminate the `plugin/skills/` vs `template/.claude/skills/` drift (Task 24 froze the template copy). Both routes pull from ONE `skills/` source; add a `validate-skill-sources.mjs` so the copies can't silently diverge again.
  - [ ] 69.3 **Slim the CLAUDE.md loader block** — move the capture *procedure* into the skill; the appended `CLAUDE.md` block shrinks to a few *facts* + a pointer ("auto-capture is on; explicit/rich capture → the memory-write skill"). Target the Anthropic <200-line guidance; stop competing with the user's own CLAUDE.md instructions. Consider `.claude/rules/` for any always-on facts.
  - [ ] 69.4* Tests: `cmk install` scaffolds `.claude/skills/memory-write/SKILL.md` (Door 2); drift validator fails when the two skill sources differ; the slimmed CLAUDE.md block is under the line budget + contains the skill pointer.
