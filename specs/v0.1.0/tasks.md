...
- [x] 80. **Auto-extract under-captures ENVIRONMENT / config facts → the `Environment Notes` section stays empty (lior-test-5 finding, 2026-06-02).** The Stop-hook auto-extract captures *preferences / doctrine* richly (venv rule, layered architecture, type-hints — all landed at `trust: high`), but **zero environment facts**: in lior-test-5 the build clearly established `port 8000`, `claude-agent-sdk 0.2.87`, and the `claude` CLI path, yet `cmk search "port 8000"` → no results and `MEMORY.md § Environment Notes` is **empty** — even though the template ships that section *for exactly this* ("Auto-extract picks these up from PostToolUse hook events"). So a designed capture path isn't firing. The classifier prompt ([auto-extract.mjs](../../packages/cli/src/auto-extract.mjs) `buildExtractionInstructions`) prioritizes durable preferences/decisions and filters config like a port number as not-durable-enough. Fix: tune the classifier to ALSO capture environment/config facts — versions, ports, URLs, tool paths, env state — and route them to `Environment Notes` (or an `environment`-typed fact). Same shape of change as Task 78 (prompt-side). Verify the `PostToolUse` observe-edit path also contributes env facts as the template promises. **Severity gated on the Session-2 recall result**: if Session 2 recalls "port 8000" cleanly (injected memory or a one-step grep), this is post-v0.2 polish; if recall fumbles, it's a fix-before-cut (env recall is core to the kit's "recall what we decided/configured" pitch — the video's L5 headline). _Requirements: recall quality (D1); design §6.0 (auto-extract) + §2.1 (Environment Notes section); surfaced by lior-test-5; relates Task 78 (classifier tuning)._
  - [ ] 80b. **Generality is asserted, not proven — strengthen the proof (audit follow-up, D-36).** The shipped change is defensibly general (the prompt names *categories*: "ports it runs on", "database names", "base URLs", "package versions"), but two weaknesses remain: (1) the unit test is **tautological** — `expect(prompt).toMatch(/port 8000/)` only checks the literal example string I typed is still present; it asserts nothing about capture *behavior*. (2) the live "port captured" win used the **same scenario** as the finding (port 8000, FastAPI), so it proves the example case, not generalization. Fix the test to assert the *category instruction* ("capture from the WORK", env/config directive present) rather than the literal `port 8000`; prove generality with a live run on a **different** config (e.g. port 3000 + Postgres) before calling env-capture done. Not a fake fix — but the evidence was weaker than presented at merge.
