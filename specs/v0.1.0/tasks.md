...
- [ ] 77. **Rename `specs/v0.1.0/` → `specs/` — the version-named folder is a misnomer (the user 2026-06-02).** `specs/v0.1.0/{requirements,design,tasks,glossary}.md` is the **living spine** (per D-19, single evolving source of truth) — it holds Tasks 63–76, design §16.19, v0.2/0.3 roadmap, etc. — all FAR past v0.1.0. The folder name is a historical artifact from when v0.1.0 was the only version; it now wrongly implies these are v0.1.0-only. Flatten to `specs/*.md` (or `specs/current/`). Mechanical but wide: `validate-references` enforces every internal link, so ALL references (across docs/, README, CLAUDE.md, the validators' path constants) must update in lockstep. Do as one atomic refactor PR. _Requirements: D-19 (living single-source spine); naming hygiene._
