...
| [`scripts/validate-index-completeness.mjs`](scripts/validate-index-completeness.mjs) | Every hand-maintained doc catalog (`docs/adr/README.md`, `docs/research/INDEX.md`, `docs/sources/README.md`, `docs/process/README.md`) lists every sibling `.md` — both directions (a missing entry AND a listed-but-deleted entry fail). Presence-only (no content judgment); per-index `exclude` allowlist | The 2026-06-15 docs-review found 4 stale indexes at once while the other doc validators stayed green (Task 152 / D-160) — converts the recurring index-lag class into a per-`npm test` check |
...
- **Watch CI on EVERY push, including direct-to-main docs commits — a red `main` is a STOP signal regardless of what changed (binding).** Direct-to-main pushes still run `ci.yml` (it triggers on `push: [main]`, not only PRs). "Docs-only, direct to main" is **not** a license to skip watching the run — `validate-doc-registry` / `validate-index-completeness` / `validate-references` / `validate-doc-completeness` all gate docs, and a doc change can turn `main` red. **Two binding sub-rules:** (a) after ANY push to `main`, confirm its CI goes green before moving on (`gh run watch` or check Actions) — do not leave `main` red and walk away; (b) when a change **adds, moves, or renames** a doc file (`docs/*.md`, `specs/*`, repo-root `*.md`, `docs/journey/*`), run the **FULL** `npm test` (or at least `npm run lint`) BEFORE pushing — the doc-structure validators are separate checks, and running a hand-picked subset (e.g. only `validate-references`) misses the others. A new `docs/*.md` / `specs/*` / repo-root `*.md` / `docs/journey/*` file MUST be registered in [`docs/DOCUMENTATION-MAP.md`](docs/DOCUMENTATION-MAP.md) in the SAME commit (`validate-doc-registry` enforces it). **Why binding (the precedent):** 2026-06-28, the v0.4.1 publish FAILED at the tag-publish step because `docs/KIRO.md` (created during the README reorg) was never registered in `DOCUMENTATION-MAP.md`. The test EXISTED and worked — `ci.yml` failed BOTH README-reorg commits on `main` at the time of push — but those were direct-to-main docs commits whose CI I didn't watch, so `main` sat red for ~4 hours until the publish surfaced it. The gap was process, not coverage: assuming docs-only-direct-to-main is safe + running only a validator subset locally. The fix was one line (register the doc), but it cost a failed publish + a force-re-tag. `P-XVB7HWGJ`.
- **After merge**:
  1. Pull main locally.
