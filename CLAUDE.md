...
- **Campaign queue**: PR-A `[MERGED]` (#32) → PR-B `[MERGED]` (#33) → PR-C `[MERGED]` (#34) → PR-D1 `audit-enforcement-validators-1` `[NEXT]` (2 validators + link-rot + docs sections) → PR-D2 `audit-enforcement-validators-2` (3 validators + annotation pass + class-5 audit + capture-turn observability) → PR-E `audit-cross-platform-portability`. Each PR runs `code-review-excellence` (ONE holistic pass per PR — see Skill agency section) before opening + `npm run stress` 5x before opening + names "Part N of 6 in the post-PR-31 audit campaign" in its description.
