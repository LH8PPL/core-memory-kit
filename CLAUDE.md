...
- **Name privacy (binding).** This repo is public. **Never write the maintainer's real name into committed/public docs** — use **"the user"** (working-style quotes, decision-log attributions, findings) or **"the maintainer"** (copyright/credit), and **"live-test"** for run labels (not "lior-test"). Applies to new DECISION-LOG entries, journey/research notes, ADRs, commit-message bodies, and code comments. Precedent: the 2026-06-04 corpus-wide de-identification sweep (D-51) replaced 537 occurrences across 54 .md files — without this rule the next session re-introduces the name (e.g. writing `Lior: "…"` into the decision log) and silently undoes the scrub. Scope is the NAME only; gendered pronouns and the git-commit author identity are out of scope (the user controls their own git config). When quoting the user, attribute to "the user".
