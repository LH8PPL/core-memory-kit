...
- **Two-pass code-review discipline (binding)** — every PR gets BOTH a self-review AND a `code-review-excellence` skill pass before merge. The two passes catch different bugs because they anchor on different mental models:
  ...
  - Lior's directive 2026-05-28: *"maybe you should also do code review of your own not just the skill after every pr before merge"*. Binding for every PR from that point on.
- **Autopilot mode (when explicitly granted)** — Lior may delegate multiple consecutive tasks with auto-merge authority. Pattern: *"do tasks N through M on autopilot, stop only for problems or my input, do code review after every task, automerge unless problems"*. When granted:
  ...
  - **Stop conditions** (autopilot pauses for Lior's input):
    - Code-review-excellence surfaces a Blocking finding that needs Lior's judgment to resolve (architectural fork, not a tactical fix)
  ...
  - **NOT stop conditions** (autopilot proceeds inline):
    ...
    - Test design
    - Fixing every code-review finding (Lior's standing "fix everything now" directive)
...
- **Strict task-order discipline (binding)** — work tasks in tasks.md order (Task N → Task N+1 → Task N+2 → …). No skipping ahead unless one of two conditions fires:
  ...
  - **"OPTIONAL" is not a skip license.** Tasks 28-35 are marked OPTIONAL in tasks.md (Layer 5 + Layer 6 — search + cron). The mark means "kit ships v0.1.0 useful without them" — NOT "skip them in autopilot ordering." Lior chose Option A for Task 33 (full cross-platform); the same standing pattern applies to all OPTIONAL tasks: ship the full feature set in numerical order.
  ...
  - Lior's directive 2026-05-28: *"do them by order, no skiping to other tasks unless you have to for the corrent task or you found a problem that needs a detour."* Binding for every task from this point on.
