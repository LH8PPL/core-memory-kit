...
> **Status:** `v0.1.2` is live on npm (provenance-signed) — one-step install, CI security, and the full recall→capture→search loop. `v0.2` (automatic cross-project persona + "Claude stays consistent") is in progress. What changed: [CHANGELOG.md](CHANGELOG.md).
...
- **Cross-project persona, built automatically** _(v0.2)_: the weekly pass notices "how you work everywhere" (tooling habits, architecture preferences) and promotes it into your **user tier** (`~/.claude-memory-kit/`) so a brand-new project already knows your style — no hand-curation. It updates itself when your preferences change and never overwrites a rule you wrote by hand.
