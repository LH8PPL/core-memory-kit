...
- **`cmk tour` — a warm tour of YOUR memory.** Narrates what's actually on disk: the three tiers and their precedence, what's been captured (real counts + a few of your own fact titles — the tour reads files, it never invents examples), how recall works, and next steps; degrades gracefully on a fresh install. Ships with in-conversation surfaces: a `/tour` slash command for Claude Code (scaffolded into `.claude/commands/`, kit-owned + refreshed on update) and a manual-inclusion `tour.md` steering file for Kiro. Tour explains; `cmk doctor` checks. (Task 175, D-215/D-363, [#309](https://github.com/LH8PPL/core-memory-kit/pull/309))
- **The kit now speaks the standard memory-taxonomy vocabulary.** README + ARCHITECTURE + glossary map the kit's surfaces to the field's four memory types (working = the scratchpads, semantic = the fact archive, episodic = the session record, procedural = habits/lessons/judgments) and name the built-in answer to each classic pitfall (graduation-not-truncation, separate fact/event stores with state labels, write-time validation, lessons-not-replays). Docs only. (Task 229, D-326/D-359, [#305](https://github.com/LH8PPL/core-memory-kit/pull/305))
- **Complete citations on every recall hit — what, WHEN, WHERE.** Every `cmk search` / `mk_search` result now carries a `date` (facts: from the fact's creation time; session-record hits: from the day-file name) and a `heading` alongside the existing id + `source_file:line` — so recall answers can say "decided 2026-06-20 under *Gateway retry policy*" instead of just quoting text. The CLI prints the date as its own column; the memory-search skill now instructs citing the when + where on every recalled item. The long-deferred `mk_remember` `cites` parameter is **deliberately closed** (permanent, documented error pointing at `links` — which already does write-time fact-linking; recall citations come from provenance automatically). (Task 227, D-326/D-358, [#304](https://github.com/LH8PPL/core-memory-kit/pull/304))
- **`cmk expand` / `mk_expand` — the recall ladder's missing middle rung.** Expand a search hit to its **source-file neighborhood**: the enclosing heading section around the hit (sibling bullets, the surrounding day-file entry), bounded — never the whole file. Works on both hit-id shapes (`P-XXXXXXXX` fact ids and `T:<file>:<line>` transcript-chunk ids); `cmk timeline` remains the chronological axis. The full ladder — search → expand → (timeline) → bodies → last-resort transcript drill, stop at the shallowest rung that answers — is now codified in the memory-search skill. (Task 226, D-326/D-356, [#303](https://github.com/LH8PPL/core-memory-kit/pull/303))
- **`cmk import-sessions` — day one isn't empty.** Bootstrap the project memory from your **existing** Claude Code session history (`~/.claude/projects/`): each past session is summarized through your agent backend into the kit's dated day-file shape ("as if captured live") and is searchable immediately, with source date + session provenance markers. **Every imported write is screened** before touching a committed tier — secret screen + PII mask + a privacy instruction in the summarize call; raw extracts archive to the local-only `context/transcripts/imported/` floor for deep recall. **Resumable + idempotent** (a killed run keeps its progress; a re-run imports only new sessions — the committed `imported-sessions.md` ledger is the resume point, not a run-once sentinel). Interactive picker with an honest cost heads-up (`--slug` / `--all-projects` / `--since` / `--max`, default 50 newest / `--all` / `--dry-run` / `--yes`), and **`cmk install` detects existing history and offers the import itself** — one question, default-skip. (Task 225, D-326/D-355, [#302](https://github.com/LH8PPL/core-memory-kit/pull/302))

### Changed

- **Internal: the four doc validators became one.** `validate-references`, `validate-doc-registry`, `validate-doc-completeness` and `validate-index-completeness` are now four **families** inside a single `scripts/validate-docs.mjs`, driven by `docs/DOCUMENTATION-MAP.md` as the manifest — one place to add a doc, no overlap, and a new **both-directions** check (a Registry entry naming a file that no longer exists now fails, not just an unregistered file). Run one family with `--only <references|registry|catalogs|coverage>`; `<!-- validate-docs: ignore -->` suppresses a line, and the legacy `<!-- validate-references: ignore -->` marker is honored forever. Dev-tooling only — the kit ships nothing from this and users never run it. (Task 186, D-249/D-370)
...
- **`cmk purge --hard <id> --yes` — the irreversible whole-fact delete.** The compliance escalation beyond `cmk forget`: removes the fact from live + every archive copy + scratchpads + its journal entry + indexes with no tombstone; a secret-free audit entry survives and the removed filenames are scrubbed from historical audit entries. Requires both flags; explicit-human-only, never an MCP tool. The git advisory prints path-scoped to exactly the purged file(s). Replaces the long-standing v0.1.x stub. (Task 96)
- **SECURITY.md gains the leaked-secret runbook** — rotate → `cmk redact` → the optional documented `filter-repo` team operation.
- **`cmk doctor` HC-12 — deletion propagation.** Verifies that a forgotten fact is *verifiably* gone from every derived surface, not just tombstoned: the search index AND the distilled summaries (`recent.md`/`archive.md`/the banked `today-*.distilled.md` artifacts). FAILs naming the exact surviving file + fact id (a summary distilled before the forget still repeating the content); surface-aware recovery — `cmk reindex` (index) or `cmk redact`/a re-distill (summary). SKIPs as explicitly *vacuous* when nothing has been forgotten yet. Alongside it, `daily-distill` now **span-replaces** already-tombstoned content with `[deleted]` (at bank-time AND at assembly) before writing a new summary — so deletions never re-enter going forward, while live facts and citations sharing a consolidated bullet are preserved. Closes the Always-On agents survey's least-implemented invariant. (Task 210, D-308)
...
- **State-labeled recall — history never masquerades as the present** — a non-current fact now arrives labeled wherever it reaches the agent: search results and `get` carry a `state` field + a `[superseded — kept for history]` / `[expired]` / `[retracted]` prefix, the session-start snapshot labels superseded bullets, and a one-line reading instruction ("unlabeled = current") rides along only when a labeled row is present — zero noise when everything is current, no flag, no LLM (a pure projection of metadata the kit already stores). Evidence basis: A-TMA's finding that identical retrieved evidence flips from wrong to correct answers on labels alone. Plus a "When recall goes wrong" bank/retrieval/QA diagnostic checklist in HEALTH-CHECKS.md. (Task 209, D-331)
- **History questions now reach the history — automatically** — a rule-based query classifier (zero LLM, negation-guarded: "not what we used before" still reads as a current question) detects history/transition questions ("what did we use *before* X", "how did Y *change*") and biases retrieval for them: expired facts auto-included, the asker's temporal hint words stripped from the keyword match, and — for a history question — the superseded/expired facts listed first, all labeled. Plain and "current" questions behave byte-identically to before. `--state-view` / `state_view` exist only as overrides; when the gate fires, a `state view: …` note explains why old rows appeared. (Task 211, D-332)
- **`cmk stats memory-health` — the memory process, observable** — five behavioral metrics aggregated from logs the kit already writes: writes-per-search, empty-search rate (with the recovered-by-retry split), redundant-write rate (writes that later dedup-merged / conflict-queued / superseded), repeated identical searches, and snapshot cap pressure — each with a week-over-week trend arrow (`--window 7|30`). Report-only: no thresholds, no pass/fail — observe before alarming. These are also the tuning numbers for the outcome-learning search blend. (Task 212, D-333)

## [0.5.2] — 2026-07-13

### Added

- **Codex support** — `cmk install --ide codex` wires OpenAI's Codex end-to-end: hooks in `.codex/hooks.json` (SessionStart recall-inject, prompt + turn capture — the turn read from the session's rollout file, edit observation, and the PreToolUse delete-guardrail), MCP registered through Codex's own `codex mcp add` (your `config.toml` is never hand-edited), a managed `AGENTS.md` block, and the automatic memory engine running through `codex exec` (read-only sandbox, your existing ChatGPT/Codex login — no API key). `--backend codex` routes split-brain background memory through Codex from any install. One-time step: run `/hooks` inside Codex once to trust the kit's hooks. Docs: `docs/CODEX.md`. (Task 196 tail, [#284](https://github.com/LH8PPL/core-memory-kit/pull/284))
...
- **A brand-new user's first cross-project rule now bootstraps the persona — the wedge fills from empty** (D-263, found live by this release's cut-gate). Before: if `~/.core-memory-kit/` didn't exist yet — every fresh machine — the Stop-hook auto-extract and `cmk lessons promote` silently dropped your first "from now on, in every project…" rule (the promote path required the user tier to already exist, so the cross-project persona could never create itself). Now the tier is scaffolded automatically on the first promotion that clears the confidence/recurrence gate — created exactly when a real durable rule is landing in it, never speculatively.
...
- **Compression now recovers from a transient Haiku failure instead of leaving the session buffer stuck.** The lifecycle compression passes (daily distill, weekly curate, the lazy session-start roll) retry once with backoff on a transient timeout / overload, so an intermittent `claude --print` slowdown no longer strands `now.md` until the next session. Failures are also now logged with their exit code + reason for diagnosis. ([#206](https://github.com/LH8PPL/core-memory-kit/pull/206))
...
- **ux(145): a session-start status line — the kit finally tells you it's working.** Every session now opens with one user-visible line (e.g. `core-memory-kit: 23 fact(s) in context, 2 captured in the last 24h, 1 conflict(s) pending — cmk queue`), shown via the hook's user-display channel: the model never sees it and it costs zero context tokens. An empty project says so honestly (`memory is empty — capture starts this session`) instead of staying silent.
...
Unify install (Task 49): a tester now needs a **single** complete entry point — `npm install -g @lh8ppl/core-memory-kit && cmk install` — with no separate `/plugin install` step. Both install routes are now complete on their own; pick one.
...
- **`.claude-plugin/marketplace.json`** at the repo root makes the plugin route registerable via `/plugin marketplace add LH8PPL/core-memory-kit` — a complete parallel entry point to `cmk install`.
...
[0.1.0]: https://github.com/LH8PPL/core-memory-kit/releases/tag/v0.1.0
