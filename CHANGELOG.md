...
- **`cmk tour` — a warm tour of YOUR memory.** Narrates what's actually on disk: the three tiers and their precedence, what's been captured (real counts + a few of your own fact titles — the tour reads files, it never invents examples), how recall works, and next steps; degrades gracefully on a fresh install. Ships with in-conversation surfaces: a `/tour` slash command for Claude Code (scaffolded into `.claude/commands/`, kit-owned + refreshed on update) and a manual-inclusion `tour.md` steering file for Kiro. Tour explains; `cmk doctor` checks. (Task 175, D-215/D-363, PR #TBD)
