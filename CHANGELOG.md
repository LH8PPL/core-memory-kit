...
- **Memory tools stay prompt-free on Claude Code 2.1.x** (Tasks 169 + 171) — recent Claude Code versions tightened how skill and MCP-tool permissions match, so the kit's allow-list now writes the `Skill(<name>:*)` wildcard form **and** each specific MCP tool (`mcp__cmk__mk_remember`, `mcp__cmk__mk_search`, …) instead of relying on the `mcp__cmk__*` server wildcard alone. Without these, the model saving a memory would hit a "Use skill?" / "proceed with mcp__cmk__mk_remember?" approval prompt — the kit's auto-capture is meant to be invisible, so it now allow-lists every form Claude Code's current matcher needs.
