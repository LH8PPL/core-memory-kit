...
- **Kiro IDE 1.0 support — new consolidated hook format + a memory delete-guard and edit-observation in the IDE.** Kiro IDE 1.0 replaced the old per-file `.kiro.hook` format with a consolidated `.kiro/hooks/cmk.kiro.hook.json` (v1). `cmk install --ide kiro` now writes **both** (the v1 file for Kiro IDE 1.0+, and the legacy files so older Kiro keeps working). The v1 format unlocks the full hook set in the IDE — recall, capture, **a delete-guardrail (`PreToolUse`, which can block a destructive command)**, and **large-edit observation (`PostFileSave`)** — so the IDE now reaches parity with Claude Code and `kiro-cli`. (The exact IDE firing behaviour is being verified against a real Kiro IDE 1.0 session; until then the v1 hooks are wired but flagged as live-unverified in the docs.)
