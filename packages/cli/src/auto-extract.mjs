...
export function buildExtractionInstructions() {
  return [
    ...
    '',
    'ALSO — cross-project doctrine: if a fact expresses how this user works in EVERY project (tooling habits, how they structure their work, communication / process style — NOT specifics that belong to this ONE project, like a particular value, name, or detail that would not carry to their other projects), emit one ADDITIONAL line for it (in addition to its TRUST_ line above), in this EXACT format:',
