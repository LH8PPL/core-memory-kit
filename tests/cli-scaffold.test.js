 ...
 */
const NON_STUB_VERBS = new Set(['version', 'install', 'uninstall', 'reindex', 'forget', 'init-user-tier', 'trust', 'search', 'remember', 'daily-distill', 'weekly-curate', 'register-crons', 'compress', 'doctor', 'digest', 'import-anthropic-memory', 'import-claude-md', 'config', 'repair', 'roll', 'disable-native-memory', 'enable-native-memory', 'get', 'timeline', 'cite', 'recent-activity', 'hook', 'cursor-hook', 'codex-hook', 'redact', 'purge', 'import-sessions', 'expand', 'tour']); // cursor-hook + codex-hook: Task 196 — wired; logic tested by cli-cursor-hook-bin.test.js. redact + purge: Task 96 (ADR-0022) — wired; require --pattern / --hard --yes (exit 2 otherwise); logic tested by cli-redact.test.js against tempdir sandboxes — never invoked unguarded from the repo cwd here (purge --hard is irreversible). import-sessions: Task 225 — wired; requires --yes non-interactively (exit 2 otherwise) and would discover the maintainer's REAL ~/.claude history from the repo cwd; logic tested by cli-import-sessions.test.js against tempdir sandboxes
